...
class DomCad_Slider_Settings {
    ...
    public function register_setting() {
        ...
        add_settings_field(
            ...
            __( 'Слайдер на главной' ), // Заголовок поля
    ...
    public function render_slider_field() {
	    ...
	    <div class="domcad-gallery-field-wrapper">
	        ...
	        <div class="domcad-gallery-preview">
	        ...
	        <p>
	            <button type="button" id="domcad-gallery_btn" class="button button-secondary">
	                <?php _e( 'Выбрать изображения для слайдера'); ?>
	            </button>
	        </p>
	    </div>
			        ...
			        $('#domcad-gallery_btn').on('click', function(e) {
			            ...
			            if (!frame) {
			                ...
			                frame.on('select', function() {
			                    var selection = frame.state().get('selection').map(function(a) { return {id: a.id , url: a.url}; });
			                    var current = $('#domcad_slider_home').val().split(',');//.filter(Boolean).map(Number);
			                    // добавляем новые, не дублируя
			                    selection.forEach(function(obj) {
			                        if (current.indexOf(obj.id) === -1) current.push(obj.id);
			                    });
